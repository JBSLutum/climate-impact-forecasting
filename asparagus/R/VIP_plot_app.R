...
labels_map <- c(
  ...
  "temp_fluctuation_risk"       = "Risiko: Schäden durch Temperaturschwünge",
  ...
  "expected_yield"                    = "Optimaler Ertrag (perfekte Bedingungen)",
  "expected_season_length"            = "Erwartete Saisonlänge",
  "season_end_doy"                    = "Ernteende (Johannitag ≈ 175)",
  #"growth_start_doy"                    = "Start Stangenwuchs (Tag des Jahres)",
  "speargrowth"                         = "Tage bis zur ersten Ernte",
  "Tsoil_mean"                          = "Ø Bodentemperatur"
)
...
VIP_plot <- function(sim_results, scen_codes = scenario_order_codes, include_unlabeled = FALSE) {
  ...
  vip_combined <- dplyr::bind_rows(vip_list) %>%
    filter(!is.na(Variable), !is.na(VIP), VIP >= 0.8)
  ...
  # ------------------------------------------------------------
  # Sortierung nach mittlerem VIP (aufsteigend = umgekehrt)
  # ------------------------------------------------------------
  
  var_order <- vip_combined %>%
    group_by(Variable) %>%
    summarise(meanVIP = mean(VIP), .groups = "drop") %>%
    arrange(meanVIP) %>%   # <-- hier geändert
    pull(Variable)
  ...
  spacers <- paste0("SPACER__", seq_along(var_order))
  spaced_levels <- as.vector(rbind(var_order, spacers))
  ...
  p <- ggplot(vip_combined, aes(x = "VIP", y = Variable_spaced)) +
    ...
    theme(
      ...
      strip.text         = element_text(hjust = 0.5, size = 12),
      axis.text.y        = element_text(size = 14, hjust = 0, vjust = 0.5),
      axis.text.x        = element_blank(),
      axis.ticks.x       = element_blank(),
      legend.position    = "bottom",
