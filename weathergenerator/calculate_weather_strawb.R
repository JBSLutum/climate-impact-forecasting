...
source('functions/weather_indices_function_strawb.R')
...
# photosynhthesis_day <-
#   purrr::map_dbl(test_out, 'photosynhthesis_day')
# risk_rain <- purrr::map_dbl(test_out, 'risk_rain')
# 
# l <-
#   purrr::map_int(test_out, function(x)
#     length(x$yday_speargrowth))
# fix_out <- which(l != 1)
# for (i in fix_out) {
...
#Summer parameters
summer_drought_stress_risk<- purrr::map_dbl(test_out, 'summer_drought_stress_risk')
summer_insect_risk<- purrr::map_dbl(test_out, 'summer_insect_risk')
summer_disease_risk<- purrr::map_dbl(test_out, 'summer_disease_risk')
summer_mean_temp<- purrr::map_dbl(test_out, 'summer_mean_temp')
pad<- purrr::map_dbl(test_out, 'pad')
#winter parameters
actual_chill<- purrr::map_dbl(test_out, 'actual_chill')
winter_mean_temp<- purrr::map_dbl(test_out, 'winter_mean_temp')
#spring parameters
#spring_start<- purrr::map_dbl(test_out, 'spring_start')
spring_pollinator<- purrr::map_dbl(test_out, 'spring_pollinator')
spring_disease_risk<- purrr::map_dbl(test_out, 'spring_disease_risk')
spring_frost_risk<- purrr::map_dbl(test_out, 'spring_frost_risk')
spring_mean_temp<- purrr::map_dbl(test_out, 'spring_mean_temp')
#fruit parameters
#fruit_start<- purrr::map_dbl(test_out, 'fruit_start')
fruit_frost_risk<- purrr::map_dbl(test_out, 'fruit_frost_risk')
fruit_drought_stress_risk<- purrr::map_dbl(test_out, 'fruit_drought_stress_risk')
fruit_hail_risk<- purrr::map_dbl(test_out, 'fruit_hail_risk')
fruit_sunburn_risk<- purrr::map_dbl(test_out, 'fruit_sunburn_risk')
fruit_mean_temp<- purrr::map_dbl(test_out, 'fruit_mean_temp')
fruit_insect_risk<- purrr::map_dbl(test_out, 'fruit_insect_risk')
fruit_disease_risk<- purrr::map_dbl(test_out, 'fruit_disease_risk')
fruit_snail_risk<- purrr::map_dbl(test_out, 'fruit_snail_risk')
#harvest parameters
#harvest_start<- purrr::map_dbl(test_out, 'harvest_start')
harvest_frost_risk<- purrr::map_dbl(test_out, 'harvest_frost_risk')
harvest_heat_risk<- purrr::map_dbl(test_out, 'harvest_heat_risk')
harvest_hail_risk<- purrr::map_dbl(test_out, 'harvest_hail_risk')
harvest_rain_risk<- purrr::map_dbl(test_out, 'harvest_rain_risk')
harvest_fly_risk<- purrr::map_dbl(test_out, 'harvest_fly_risk')
harvest_mean_temp<- purrr::map_dbl(test_out, 'harvest_mean_temp')
...
risk_df_strawb <- data.frame(
  ...
  mutate(id = 1:length(harvest_fly_risk))
...
id <- 1495
risk_df_strawb$summer_disease_risk[id]

write.csv(risk_df_strawb,"weathergenerator/risk_df_strawb.csv")
